...
export function GENERATE_ENTRY_PROMPT(existingTitles, seedHint) {
  let hintText = '新しい困ったおじさんのタイプ（アンチパターン）を考えてください。説教、武勇伝、マナー違反、時代錯誤な価値観など、「こんな大人にはなりたくない」と思わせつつも、どこか憎めない独自の個性を持つものを創造してください。';
  if (seedHint) {
    hintText = `テーマのヒント: ${seedHint}`;
  }

  let existingText = '';
  if (existingTitles.length > 0) {
    existingText = `既存のエントリー（なるべく別の方向性で考えてください）:\n${existingTitles.map(function(t) { return `- ${t}`; }).join('\n')}`;
  }
